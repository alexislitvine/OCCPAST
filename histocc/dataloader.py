...
class OccDatasetMixerInMemMultipleFiles(OccDatasetV2):
    def __init__(
        ...
        """
        Dataset class for training with multiple input files.

        Parameters
        ----------
        include_descriptions : bool, default=False
            If True, randomly includes the code description in the input during
            training. This can help the model learn contextual associations between
            occupational descriptions and their standardized codes.
        description_prob : float, default=0.5
            Probability of including the description in the input when
            `include_descriptions=True`. Only applies during training.
        description_template : str, default="{occ} [DESC: {desc}]"
            Template for formatting the input when descriptions are included.
            Use {occ} for the occupational description and {desc} for the
            code description.
        descriptions : dict, optional
            Pre-loaded dictionary mapping codes to descriptions. If provided,
            this takes precedence over descriptions_file. Can be either:
            - Simple dict: {code: description}
            - Multilingual dict: {code: {lang: description}}
        descriptions_file : str, optional
            Path to a CSV file containing code-to-description mappings.
            Must specify descriptions_code_col and descriptions_text_col.
        descriptions_code_col : str, optional
            Column name in descriptions_file containing the codes.
        descriptions_text_col : str, optional
            Column name in descriptions_file containing the descriptions.
        descriptions_lang_col : str, optional
            Column name in descriptions_file containing the language codes.
            If provided, descriptions will be loaded as {code: {lang: description}}
            and the appropriate language will be used based on each record's lang.
        """
        if isinstance(target_cols, str):
            target_cols = self.map_type_target_cols_default[target_cols]
        ...
        self.descriptions_multilingual = descriptions_lang_col is not None

        # Load descriptions from various sources
    ...
    def _load_descriptions(
        ...
        """Load descriptions from the provided source."""
        if not self.include_descriptions:
            return None

        # Priority 1: Pre-loaded dictionary
        if descriptions is not None:
            # Check if it's already a multilingual dict (nested structure)
            first_value = next(iter(descriptions.values()), None)
            if isinstance(first_value, dict):
                # Already multilingual format {code: {lang: description}}
                self.descriptions_multilingual = True
                return {str(k): v for k, v in descriptions.items()}
            else:
        ...
        if descriptions_file is not None:
            if descriptions_code_col is None or descriptions_text_col is None:
                raise ValueError(
                    "When using descriptions_file, must specify both "
                    "descriptions_code_col and descriptions_text_col"
                )
            from .utils.descriptions import load_descriptions_from_csv
            return load_descriptions_from_csv(
                descriptions_file,
                code_col=descriptions_code_col,
                desc_col=descriptions_text_col,
                lang_col=descriptions_lang_col,
            )

        # Priority 3: No descriptions provided - return empty dict
        # (don't auto-load HISCO since user might be using PST2 or other system)
        return {}
    ...
    def _get_primary_description(self, record: pd.Series) -> str | None:
        """
        Get the description for the primary (first) target code.

        For multilingual descriptions, uses the record's language to look up
        the appropriate description.

        Parameters
        ----------
        record : pd.Series
            A single record from the dataframe.

        Returns
        -------
        str | None
            The description for the primary code, or None if not found.
        """
        if not self.include_descriptions or not self.descriptions:
            return None

        # Get the first target column value (primary code)
        first_target_col = self.target_cols[0]
        code = record[first_target_col]
        ...
        code_descriptions = self.descriptions.get(code_str)
        if code_descriptions is None:
            return None

        # Handle multilingual descriptions
        if self.descriptions_multilingual and isinstance(code_descriptions, dict):
            lang = record.lang if hasattr(record, 'lang') and pd.notna(record.lang) else 'en'
            # Try exact match first, then fall back to 'en', then first available
            if lang in code_descriptions:
                return code_descriptions[lang]
            elif 'en' in code_descriptions:
                return code_descriptions['en']
            elif code_descriptions:
                return next(iter(code_descriptions.values()))
            return None
        else:
    ...
    def __getitem__(self, item: int) -> dict[str, str | Tensor]:
        ...
        if (self.include_descriptions and self.training and
            random.random() < self.description_prob):
            description = self._get_primary_description(record)
            if description is not None:
                occ_descr = self.description_template.format(
                    occ=occ_descr,
                    desc=description
                )
