...
class OccCANINE:
    ...
    def _list_of_formatted_codes(self, codes_list: list[str] | None = None):
        ...
        This method handles both single codes (e.g., '12345') and multi-code 
        sequences (e.g., '12345&67890') by extracting the appropriate number 
        of tokens for each code in the sequence.
        """
        # Get codes
        ...
        codes_list = [i for i in codes_list if i != " " and i.lower() != "nan" and "TBD" not in i.upper()]
        
        # For each code string, determine how many codes it contains and extract
        # the appropriate number of tokens. Multi-code sequences (e.g., '12345&67890')
        # need more tokens than single codes.
        formatted_codes = []
        for code_str in codes_list:
            # Count number of codes in this string
            sep_value = getattr(self.formatter, 'sep_value', '')
            if sep_value:
                num_codes = len(code_str.split(sep_value))
            else:
                num_codes = 1
            
            # Calculate how many tokens to extract (code_len tokens per code)
            tokens_needed = num_codes * self.code_len
            
            # Transform and extract the appropriate number of tokens (skip BOS at index 0)
            transformed = self.formatter.transform_label(code_str)
            formatted_codes.append(transformed[1:1+tokens_needed])
    ...
    def predict(
        ...
        - debug (bool): If True, prints raw model outputs for debugging purposes. When using greedy decoding, this will print the first 10 raw predictions (pred_s2s) and the formatter's separator value. This is useful for diagnosing issues with code splitting or lookup failures.
    ...
    def _predict_greedy(self, data_loader, order_invariant_conf):
        ...
        # Debug: Show raw token sequences before cleaning
        if getattr(self, '_debug', False):
            formatter = data_loader.dataset.formatter
            print("\n=== DEBUG: Multi-Occupation Diagnostic ===")
            print(f"Formatter config:")
            print(f"  - block_size: {formatter.block_size}")
            print(f"  - max_num_codes: {formatter.max_num_codes}")
            print(f"  - sep_value: {formatter.sep_value!r}")
            # within_block_sep may not exist on older formatters (e.g., hisco_blocky5)
            print(f"  - within_block_sep: {getattr(formatter, 'within_block_sep', None)!r}")
            print(f"\nRaw token sequences (first 3):")
            for i, raw_seq in enumerate(preds_s2s_raw[:3]):
                # Show the sequence structure
                blocks = []
                for block_idx in range(formatter.max_num_codes):
                    start = 1 + block_idx * formatter.block_size
                    end = start + formatter.block_size
                    block = raw_seq[start:end]
                    is_pad = (block == PAD_IDX).all()
                    blocks.append(f"Block{block_idx+1}: {'[PAD]' if is_pad else block.tolist()}")
                print(f"  Sample {i+1}: {' | '.join(blocks)}")
            print("==========================================\n")
    ...
    def _format(
        ...
        if out_type == "probs":
            ...
            if what == "probs":
                ...
                # Debug: print raw model outputs if debug flag is set
                if getattr(self, '_debug', False):
                    print("\n=== DEBUG: Raw Model Outputs ===")
                    print(f"Raw pred_s2s (first 10):\n{out.pred_s2s.head(10).tolist()}")
                    print(f"Formatter sep_value: {self.formatter.sep_value!r}")
                    print("================================\n")
                ...
                if what == "bin":
                    ...
                    for i, item in enumerate(processed_data):
                        for sub_item in item:
                            # Normalize the code to match the key format (strips trailing zeros for PST)
                            if isinstance(sub_item, str):
                                normalized = self._normalize_code_for_lookup(sub_item, inv_key)
                                if normalized in inv_key:
                                    res_bin[i, inv_key[normalized]] = 1
                            elif sub_item in inv_key:
                    ...
                    for item in processed_data:
                        codes = []
                        for sub_item in item:
                            # Handle np.nan and other invalid values properly
                            if isinstance(sub_item, float) and np.isnan(sub_item):
                                codes.append('unan')  # Mark as unknown/nan
                            elif str(sub_item).strip() == '' or str(sub_item).strip() == '0':
                                # Empty string or "0" should be treated as nan
                                codes.append('unan')
                            else:
                                # Normalize the code to match the key format (strips trailing zeros for PST)
                                normalized = self._normalize_code_for_lookup(str(sub_item), inv_key)
                                if normalized in inv_key:
                                    codes.append(inv_key[normalized])
                                else:
                                    codes.append(f'u{sub_item}')  # Add 'u' to ensure being able to pick it up in cleaning below

                        # Build result row: for known codes, use key and description; for unknown, show the raw code
                        row = []
                        for code_idx in codes:
                            if code_idx in self.key:
                                row.append([self.key[code_idx], self.key_desc[code_idx]])
                            elif isinstance(code_idx, str) and code_idx.startswith('u'):
                                row.append([code_idx[1:], "Unknown code"])  # Strip 'u' prefix and mark as unknown
                            else:
                                row.append([np.nan, "Unknown code"])
                        row = [item for sublist in row for item in sublist]  # Flatten list
                        res.append(row)
    ...
    def _normalize_code_for_lookup(self, code: str, inv_key: dict | None = None) -> str:
        """
        Normalize a predicted code to match the format used in self.key.
        
        For systems with within_block_sep (like PST), this strips trailing ',0' 
        parts from codes to match the key format. For example, '1,5,6,1,11,0,0,0' 
        becomes '1,5,6,1,11' if the key stores codes without trailing zeros.
        
        For systems without within_block_sep (like HISCO), this returns the code unchanged.
        
        Parameters:
        - code: The code string to normalize
        - inv_key: Optional inverted key dictionary (values -> keys). If not provided, 
                   it will be computed from self.key. For better performance when processing
                   many codes, compute inv_key once and pass it to each call.
        """
        if not self.use_within_block_sep:
            return code
        
        # For systems with within_block_sep, try to find the code in inv_key
        # by progressively stripping trailing ',0' parts
        if inv_key is None:
            inv_key = dict(map(reversed, self.key.items()))
        ...
        # Try stripping trailing ',0' parts
        parts = code.split(',')
